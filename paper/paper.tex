...
\renewcommand{\topfraction}{0.95}\renewcommand{\bottomfraction}{0.9}\renewcommand{\textfraction}{0.05}\renewcommand{\floatpagefraction}{0.85}
\usepackage{caption}
...
\begin{abstract}
Engineering databases are merged from laboratories, sites and campaigns, each of which
recorded its own subset of the features with its own calibration; their gaps are
block-structured and informative about the source, not random. Tabular foundation
models treat missing values cell by cell, and we show that this is where they all lose:
when the test rows come from a source the context has never seen, TabICLv2, TabPFN~2.5,
TabPFN-3, classical imputers and gradient-boosted trees give up 0.02--0.08 AUC or 1--3
RMSE beyond what the gaps cost, and none recovers it. TabICL-M continues TabICLv2 on
one observation: rows that share a missingness pattern come from the same source, so the
pattern is free, label-free provenance. It encodes each value relative to its own
pattern group, represents each row from its observed features only, carries a pattern
token into in-context learning, and is trained with a source-structured missingness
prior, block reconstruction and an offset-consistency loss; every addition is
zero-initialised, so on complete data it equals TabICLv2. On 20 datasets with injected
block missingness (5 seeds, held-out-source and random splits) TabICL-M is the best
model of any kind when a held-out source carries an offset (mean rank 2.22 against 2.45
for TabPFN-3; 14 of 20 datasets; Elo 1325 against 1267 under TabArena's scoring), level
with TabPFN-3 on classification, and behind it only on plain regression, at half the
parameters and a third of the inference time. Every negative result on the way is
reported, so that the remaining gap is attributed to the base regressor, not to the
method.
\end{abstract}
...
and up to 100 features, Muon optimiser \citep{jordan2024muon}, cosine schedule, learning rate $5\times
...
(\texttt{results/broad}); the datasets are listed in Appendix~\ref{app:data} and the per-dataset numbers in Appendix~\ref{app:perdataset}.
...
there too, so it is not capacity. \emph{A function-class diagnostic} on synthetic
tasks of known class (GP samples, sine MLP, products, exponentials of a linear form,
kinematic chains, step functions; 1 and 8 inputs; 300 to 3\,000 rows; three seeds)
locates the gap: $+128\,\%$ RMSE on exponentials and $+115\,\%$ on products of several
features, $+6$ to $+12\,\%$ on the other nonlinear families, none on linear targets;
$+1\,\%$ with one input against $+12\,\%$ with eight; unchanged from 300 to 3\,000 rows.
TabICLv2 and TabICL-M are identical on every family, so the missingness training is not
the cause. \emph{A neighbour-based context} (the 4\,000 training rows nearest to each
group of test rows, as in LoCalPFN and TabDPT) on kin8nm, space\_ga and diamonds at up to
14\,000 training rows loses to the full context (1 win, 11 losses, $+0.5\,\%$ RMSE).
What remains is the quality of the base regressor on interactions of several features, inherited from TabICLv2 (whose released regressor ranks
...
\paragraph{What a fair comparison required.} Three design choices decided what could
be claimed. Pairing every comparison on identical splits and deleted cells turned
seed-level noise into countable wins and losses. Splitting by task showed that the
pooled second place behind TabPFN-3 (2.32 against 2.28) is entirely regression without a
source offset, with classification level. And probing that deficit with every lever we
could think of (Section~\ref{sec:negative}) placed it in the base regressor rather than in
the method. Without any one of these the paper would have claimed either too much or the
wrong thing.
...
we target; on TabArena-Lite (51 tasks, eight-fold bagging) TabICL-M scores Elo 1500
($+60/-48$) against 1557 for TabICLv2 and 1638 for TabPFN-3, inside TabICLv2's interval
but below it, as expected on complete data; the BeyondArena grouped-split run is
reported in the repository. Finally, one RTX~5090 and 20\,000 continuation steps are a small budget
...
\paragraph{Reproducibility.} The repository contains the weights (git LFS), the prior, the
training scripts with every switch used here, the evaluation runner, the raw per-fit
results of every experiment in this paper (\texttt{results/}), the scripts that produce
each table and figure from them, and the pipelines that ran the training and evaluation
unattended. Seeds, splits and deleted cells are deterministic functions of the seed, so
every paired comparison can be recomputed. The hyperparameters are in
...
Twenty public datasets, subsampled to at most 3\,000 rows before the 70/30 split; train
rows are the median over seeds and splits. Classification metric AUC (log loss for the
TabArena-style scoring), regression metric RMSE.

\begin{table}[!htbp]
\centering\small
\caption{The 20 datasets of the benchmark.}
...
Mean over 5 seeds at 50\,\% missingness; AUC for classification (higher is better), RMSE
for regression (lower is better); best per row in bold.

\begin{table}[!htbp]
\centering\small
\caption{Held-out source with offset (block\_shift, rate 0.5).}
\input{tables/per_dataset_loso_shift.tex}
\end{table}

\begin{table}[!htbp]
\centering\small
\caption{Held-out source without offset (block, rate 0.5).}
\input{tables/per_dataset_loso_block.tex}
\end{table}

\begin{table}[!htbp]
\centering\small
\caption{Random split with offset (block\_shift, rate 0.5).}
\input{tables/per_dataset_random_shift.tex}
\end{table}

\begin{table}[!htbp]
\centering\small
\caption{Complete data (no deleted cells), random split.}
\input{tables/per_dataset_complete.tex}
\end{table}
\FloatBarrier
...
\caption{The two stages that produced the shipped checkpoints. Both start from the
previous weights with a fresh cosine schedule (5\,\% warm-up).}
\begin{tabular}{lcc}
\toprule
& stage 4 & stage 4b\\
\midrule
steps & 10\,000 & 10\,000\\
learning rate & $5\times10^{-5}$ & $3\times10^{-5}$\\
...
optimiser & Muon ($\beta_1$ 0.9, wd 0.01, clip 1.0) & same\\
precision & bfloat16 autocast & bfloat16 autocast\\
reconstruction & mode mixed, weight 0.1, up to 30\,\% of cells, $p=0.5$ & same\\
offset consistency & weight 0.1, $p=0.25$, tables $\le 2\,048$ rows & same, shift $\le 1.2\sigma$\\
\midrule
...
prior: per-source offset & $p=0.8$, up to $0.5\sigma$ & $p=1.0$, up to $1.2\sigma$\\
prior: per-source noise & $p=0.6$, up to $0.3\sigma$ & $p=0.8$, up to $0.5\sigma$\\
...
K. Jordan, Y. Jin, V. Boza, J. You, F. Cesista, L. Newhouse, J. Bernstein.
