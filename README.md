...
Verdict (best of ours against TabPFN-3 at its best setting): **AHEAD when a held-out source
carries a measurement offset** (49 wins / 21 losses, 4 of 7 datasets, 2.1 % lower RMSE on average),
**BEHIND without an offset** (21/49) and **on the random split** (30/54, 0.6 % higher RMSE).
That is the same boundary as for classification: the source-aware parts pay where a source shifts,
and base-model strength decides everywhere else.
...
4. **Base regression strength.** TabPFN-3 leads on regression wherever no source
   ...
   (see the regression section above), so closing it needs a stronger base
   regressor, not more missingness training.
