...
Regression is where TabPFN-3 leads. The levers tried, hands-off: 32 ensemble members (`_n32`), the median instead of the mean (`_med`), a target power-transform ensemble (`_ypow`), an extra `quantile` normalisation member (`_qn`), a regressor continued on a half-complete prior (`_4c`) and, if still behind, on a mostly complete prior (`_4d`). Full tables: `results/regression/summary.md`.
