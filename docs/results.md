...
All result tables, moved out of the README. The interpretation is in the README
section *Source-aware TabICL-M: what was done and what it shows*; the training
details are in [training.md](training.md). Every table below is produced by
`scripts/ablation_missingness.py` and the analysis scripts next to the result
directories; the part between the markers is regenerated by
